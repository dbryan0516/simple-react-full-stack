...
mapboxgl.accessToken = 'pk.eyJ1IjoiZGJyeWFuIiwiYSI6ImNrM2RwZ3VlYzAwNmwzaHAxNWZraXcxMzkifQ.2ZkrYlzMuItCBWiF5--Ywg';

class MapBox extends React.Component {

  constructor(props) {
    super(props);
    var defaultState = 'North Carolina';
    var selectedState = States[defaultState];
    this.state = {
      username: "dylan",
      long: selectedState.long,
  ...
  componentDidMount() {
    // TODO: calculate center and zoom based on what state/region
    // todo: is selected; maybe a dictionary of some form?
    let map = new mapboxgl.Map({
      container: 'mapboxy',
      style: 'mapbox://styles/dbryan/ck3kj4yd80sqt1cpeq3dw4h41',
    ...
    map.on('load', function () {
      ...
      map.on('click', 'places', function (e) {
        ...
        // TODO: for set html can I use a component here and pass parameters?
        // TODO: or some other kind of populated variable
        new mapboxgl.Popup()
...
// Change it back to a pointer when it leaves.
  ...
  handleStateSelection = selection => {
    debugger;
    if (States[selection.value] != null) {
      var selectedState = States[selection.value];
      this.setState({
        selectedState: selection.value,
        long: selectedState.long,
        lat: selectedState.lat
      });
    }
  ...
  return (<div>
    ...
    <p>{this.state.selectedState}</p>
            <div id="mapboxy" className="mapboxy"></div>
          </div>);
